...
{
    internal class JiliCancelSessionBetService : JiliBaseActionService<JiliCancelSessionBetIpo, JiliCancelSessionBetDto>
    {
        protected override ProviderAction Action => ProviderAction.Rollback;
        public JiliCancelSessionBetService(string providerId, JiliCancelSessionBetIpo ipo) : base(providerId, ipo)
        {
        }
        ...
        protected override async Task<AppLoginTokenDO> GetLoginTokenDo()
        {
            ...
            if (referEo != null && referEo.RoundId != Ipo.sessionId.ToString())
                throw new CustomException(JiliResponseCodes.RS_OtherError.ToString(), $"Rollback时被回滚订单roundId不相同. referOrderId:{Ipo.round.ToString()} ipo.roundId:{Ipo.sessionId} refer.roundId:{referEo.RoundId}");
        ...
        protected override async Task Execute(JiliCancelSessionBetDto dto)
        {
            // 回滚订单不存在，直接返回
